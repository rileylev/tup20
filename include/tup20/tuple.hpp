...
constexpr auto size = [] {
  ...
  if constexpr(impl::has_size_member<Tup_>) return Tup_::size;
...
using nth_t = decltype([] {
  ...
  if constexpr(impl::has_nth_t_member<N, Tup_>)
    return std::declval<
        impl::copy_cv<Tup_, typename Tup_::template nth_t<N>>>();
...
namespace impl { namespace niebloid {
  using std::get;
...
}} // namespace impl::niebloid

template<class T>
inline constexpr impl::niebloid::get_t<T> get_t{};
template<auto N>
inline constexpr impl::niebloid::get_n<N> get_n{};
...
namespace impl {
   ...
   * box<0,T0>, box<1,T1>...box<n,Tn>. This creates n member variables
   * called elt to store each type. It lets us implement get via overloads
   * on box. (See tuple_friends::{get_n_,get_t_})
   */
...
#  define TUP20_COMMA_NTH_TYPE(tup, N)
    ...
    // implement get with helpful error messages. We need overload
    // resolution to conditionally error with the helpful message. I think
    // it makes more sense to search through static methods for this, not
    // friend functions/adl
...
namespace impl {
  ...
  constexpr auto equal_(X const& x, Y const& y, std::index_sequence<I...>)
...
namespace impl {
  ...
  struct cat_times_friend {
    friend constexpr auto operator*(auto left, auto right) TUP20_ARROW(
        cat_times{left.make, cat2(left.tup, right.tup, left.make)})
  };
...
consteval auto cat_for(auto make_tup)
    TUP20_RET([make_tup](auto&&... tups) TUP20_NOEX(
        (impl::cat_times{make_tup, TUP20_FWD(tups)} * ...).tup))
// ARROW crashes irony
...
namespace impl {
  template<auto... I>
  constexpr auto apply_(std::index_sequence<I...>, auto&& f, auto&& tup)
      TUP20_ARROW(std::invoke(TUP20_FWD(f), get_n<I>(TUP20_FWD(tup))...))
}
constexpr auto apply(auto&& f, auto&& tup) TUP20_ARROW(impl::apply_(
    std::make_index_sequence<size<decltype(tup)>>{},
    TUP20_FWD(f),
    TUP20_FWD(tup)))
